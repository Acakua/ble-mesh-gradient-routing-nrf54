 ...
 * sensor readall
 * --------------------------------------------------------------- */
static int cmd_sensor_readall(const struct shell *sh, size_t argc, char **argv)
{
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);

    shell_print(sh, "Reading all sensors and sending telemetry...");

    sensor_packet_t pkt;
    build_sensor_packet(&pkt);

    if (pkt.count == 0) {
        shell_warn(sh, "No sensors registered.");
        return 0;
    }

    int err = bt_mesh_gradient_srv_sensor_data_send(&gradient_srv, &pkt);
    if (err == 0) {
        shell_print(sh, "OK: Sent telemetry for %d sensors", pkt.count);
    } else {
        shell_error(sh, "ERR: Failed to send (%d)", err);
    }
    return err;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(sensor_cmds,
    ...
    SHELL_CMD_ARG(readall, NULL, "Read all and send Mesh", cmd_sensor_readall, 1, 0),
