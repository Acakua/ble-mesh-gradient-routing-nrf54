...
static void cleanup_handler(struct k_work *work)
{
    ...
    /*Cập nhật lại Gradient theo Node có Gradient tốt nhất trong bảng định tuyến*/
    if (table_changed) {
        if (g_gradient_srv->forwarding_table[0].addr != BT_MESH_ADDR_UNASSIGNED) {
            ...
            if (best_parent_gradient != 255) {
                uint8_t old_gradient = g_gradient_srv->gradient;
                g_gradient_srv->gradient = best_parent_gradient + 1;
...
/* Forward declaration  - khai báo trước ở đây*/
