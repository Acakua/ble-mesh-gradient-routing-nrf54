...
static bool is_sensor_test_active =
    false; /* [NEW] Manual toggle via Button 3 */
...
static uint32_t g_stress_tx_count =
    0; /* [NEW] Counter for downlink stress test */

// [NEW] Timer để tự tắt Attention Mode sau 1 giây
static struct k_work_delayable attention_off_work;
...
static void attention_off_handler(struct k_work *work);  /* [NEW] */
...
static void handle_data_received(struct bt_mesh_gradient_srv *srv,
  ...
  /* Payload đặc biệt: 0xFFFC = Attention Mode (bật 1 giây rồi tự tắt) */
  if (data == 0xFFFC) {
    LOG_INF(">> ATTENTION MODE activated (1s) <<");
    led_indicate_attention(true);
    k_work_reschedule(&attention_off_work, K_MSEC(1000));
    return;
  }
  ...
  if (data == 0) {
    ...
    dk_set_led(DK_LED1, led1_state);
  } else if (data == 1) {
    LOG_INF("Manual Toggle: LED 2 (Data: 1)");
    static bool led2_state = false;
    led2_state = !led2_state;
    dk_set_led(DK_LED2, led2_state);
  } else if (data == 3) {
    LOG_INF("Manual Toggle: LED 4 (Data: 3)");
    static bool led4_state = false;
    led4_state = !led4_state;
    dk_set_led(DK_LED4, led4_state);
  } else {
    /* Trường hợp payload không xác định */
    LOG_WRN("Unknown payload received: %d. No action taken.", data);
  }
}
...
static void handle_report_req_received(struct bt_mesh_gradient_srv *srv) {
  ...
  LOG_INF(">> Report will be sent in %d ms (Reliable Sequence)...",
          random_delay);
...
const struct bt_mesh_comp *model_handler_init(void) {
  ...
  /* [NEW] Attention Mode Auto-off */
  k_work_init_delayable(&attention_off_work, attention_off_handler);
...
void sink_stop_test(void) {
  ...
  if (was_stress) {
    ...
    LOG_INF("Sending Downlink Report to 0x%04x (TX: %u)...", stress_target_addr,
            g_stress_tx_count);
    bt_mesh_gradient_srv_send_downlink_report(&gradient_srv, stress_target_addr,
                                              (uint16_t)g_stress_tx_count);
  }
...
static void attention_off_handler(struct k_work *work) {
  LOG_INF(">> ATTENTION MODE timeout: turning OFF <<");
  led_indicate_attention(false);
}
...
static void send_data_handler(struct k_work *work) {
  ...
  if (has_route) {
    ...
    /* [NEW] Khởi tạo Min RSSI = 0 (giá trị lớn nhất) cho gói tin gốc (Timestamp
     * removed) */
    int err = bt_mesh_gradient_srv_data_send(&gradient_srv, dest_addr,
                                             g_total_tx_count, 0);
    if (err) {
...
static void print_neighbor_table(void) {
  ...
  for (int i = 0; i < CONFIG_BT_MESH_GRADIENT_SRV_FORWARDING_TABLE_SIZE; i++) {
    ...
    if (e->addr != BT_MESH_ADDR_UNASSIGNED) {
      ...
      LOG_INF("| %3d | 0x%04x | %4d | %4d | %10lld | %s |", i, e->addr,
              e->gradient, e->rssi, (now - e->last_seen), status);
    }
  }
  ...
  if (has_changed & button_state & DK_BTN3_MSK) {
    ...
    if (gradient_srv.gradient != 0) {
      ...
      if (!is_sensor_test_active) {
        ...
        struct packet_stats stats;
        pkt_stats_get(&stats);
        LOG_INF("Local Test Stopped. TX: %u, RX Backprop: %u", stats.data_tx,
                stats.rx_data_count);
      }
      return;
    }
...
static void stress_tx_handler(struct k_work *work) {
  ...
  int err = bt_mesh_gradient_srv_backprop_send(
      &gradient_srv, stress_target_addr, g_total_tx_count);
  if (err) {
...
static void stress_timeout_handler(struct k_work *work) {
  ...
  LOG_INF("Sending Final Downlink Report to 0x%04x (TX: %u)...",
          stress_target_addr, g_stress_tx_count);
  bt_mesh_gradient_srv_send_downlink_report(&gradient_srv, stress_target_addr,
                                            (uint16_t)g_stress_tx_count);
}