...
/** Marker value used in DATA packets to identify heartbeat frames (if no sensors) */
...
/**
 * @brief Work handler — sends one SENSOR_DATA (0x18) or legacy Heartbeat (0xFFFF)
 */
static void heartbeat_work_handler(struct k_work *work)
{
    ...
    LOG_DBG("[SensorData] Sending periodic packet, interval=%ums",
            g_sensor_interval_ms);

    /* 1. Get sensor data from manager */
    sensor_packet_t pkt;
    build_sensor_packet(&pkt);

    int err;
    if (pkt.count > 0) {
        /* Send multi-sensor packet (Opcode 0x18) */
        err = bt_mesh_gradient_srv_sensor_data_send(heartbeat_srv, &pkt);
    } else {
        /* Send legacy empty heartbeat (Opcode 0x0B with 0xFFFF) if no sensors configured */
        uint16_t dummy_addr = 0;
        err = data_forward_send_direct(heartbeat_srv, dummy_addr,
    ...
    /* Reschedule with configured interval */
