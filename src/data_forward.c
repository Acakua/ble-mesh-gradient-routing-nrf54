...
struct data_send_context {
    ...
    uint16_t data;
    uint16_t sender_addr;      /* Original sender address (to skip) */
                              ...
                              uint16_t addr, uint16_t data)
{
    ...
    BT_MESH_MODEL_BUF_DEFINE(buf, BT_MESH_GRADIENT_SRV_OP_DATA_MESSAGE, 2);
...
static void data_retry_handler(struct k_work *work)
{
    ...
    while (ctx->current_index < CONFIG_BT_MESH_GRADIENT_SRV_FORWARDING_TABLE_SIZE) {
        ...
        int err = data_send_internal(gradient_srv, next_addr, ctx->data);
                      ...
                      uint16_t data, uint16_t sender_addr)
{
    ...
    while (data_send_ctx.current_index < CONFIG_BT_MESH_GRADIENT_SRV_FORWARDING_TABLE_SIZE) {
        ...
        LOG_INF("[Forward] Sending data %d to 0x%04x (index %d)", 
                data, dest_addr, data_send_ctx.current_index);
        
        int err = data_send_internal(gradient_srv, dest_addr, data);
                             ...
                             uint16_t addr, uint16_t data)
{
    ...
    return data_send_internal(gradient_srv, addr, data);
}
