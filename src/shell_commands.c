...
static int cmd_mesh_info(const struct shell *sh, size_t argc, char **argv) {
  ...
  shell_print(sh, "Vai tro    : %s",
              (gradient == 0) ? "GATEWAY" : "REGULAR NODE");
  shell_print(sh, "Heartbeat  : %s",
...
static int cmd_mesh_fwd(const struct shell *sh, size_t argc, char **argv) {
  ...
  for (int i = 0; i < CONFIG_BT_MESH_GRADIENT_SRV_FORWARDING_TABLE_SIZE; i++) {
    if (gradient_srv.forwarding_table[i].addr != BT_MESH_ADDR_UNASSIGNED) {
      int64_t age_sec =
          (now - gradient_srv.forwarding_table[i].last_seen) / 1000;
      shell_print(sh, "[%d] addr=0x%04x  gradient=%d  rssi=%d  age=%lld sec", i,
                  gradient_srv.forwarding_table[i].addr,
                  gradient_srv.forwarding_table[i].gradient,
                  gradient_srv.forwarding_table[i].rssi, age_sec);
      has_entry = true;
    }
  }
...
static int cmd_mesh_dest(const struct shell *sh, size_t argc, char **argv) {
  ...
  for (int i = 0; i < CONFIG_BT_MESH_GRADIENT_SRV_FORWARDING_TABLE_SIZE; i++) {
    ...
    struct backprop_node *node = gradient_srv.forwarding_table[i].backprop_dest;
    while (node != NULL) {
      shell_print(sh, "  0x%04x  (via nexthop 0x%04x)", node->addr,
                  gradient_srv.forwarding_table[i].addr);
      count++;
      node = node->next;
    }
...
static int cmd_mesh_backprop(const struct shell *sh, size_t argc, char **argv) {
  ...
  /* Gửi BACKPROP */
  int err = bt_mesh_gradient_srv_backprop_send(
      &gradient_srv, (uint16_t)dest_addr, (uint16_t)payload);
...
static int cmd_mesh_data(const struct shell *sh, size_t argc, char **argv) {
  ...
  shell_print(sh, "");
  shell_print(sh, "Gui DATA voi payload=%lu qua nexthop 0x%04x...", payload,
              nexthop);

  /* Gửi DATA */
  int err = bt_mesh_gradient_srv_data_send(&gradient_srv, nexthop,
                                           (uint16_t)payload, 0);
 ...
 */
static int cmd_mesh_heartbeat(const struct shell *sh, size_t argc,
                              char **argv) {
  ARG_UNUSED(argc);
  ARG_UNUSED(argv);
 ...
 */
static int cmd_mesh_report_stop(const struct shell *sh, size_t argc,
                                char **argv) {
  if (argc < 2) {
 ...
 */
static int cmd_mesh_stress_dl(const struct shell *sh, size_t argc,
                              char **argv) {
  if (argc != 2) {
 ...
 */
static int cmd_mesh_stats_show(const struct shell *sh, size_t argc,
                               char **argv) {
  ARG_UNUSED(argc);
  ARG_UNUSED(argv);
 ...
 */
static int cmd_mesh_stats_reset(const struct shell *sh, size_t argc,
                                char **argv) {
  ARG_UNUSED(argc);
  ARG_UNUSED(argv);
...
/*============================================================================*/
/*                         Command: mesh attention                            */
/*============================================================================*/

/**
 * @brief Gửi lệnh Attention Mode đến node đích qua BACKPROP
 *
 * Lệnh: mesh attention <dest_addr>
 *
 * Tham số:
 *   - dest_addr: Địa chỉ đích (hex hoặc decimal)
 *
 * Node đích sẽ bật Attention Mode (LED xoay vòng) trong 1 giây rồi tự tắt.
 * Chỉ Gateway (gradient=0) mới có thể gửi lệnh này.
 */
static int cmd_mesh_attention(const struct shell *sh, size_t argc, char **argv) {
  if (argc != 2) {
    shell_error(sh, "Sai cu phap!");
    shell_print(sh, "Dung: mesh attention <dest_addr>");
    shell_print(sh, "Vi du: mesh attention 0x0003");
    return -EINVAL;
  }
  ...
  /* Kiểm tra có phải Gateway không */
  if (gradient_srv.gradient != 0) {
    shell_error(sh, "Chi Gateway (gradient=0) moi co the gui Attention!");
    return -ENOEXEC;
  }
  ...
  shell_print(sh, "");
  shell_print(sh, "Gui ATTENTION MODE den 0x%04x...", (uint16_t)dest_addr);

  /* Gửi BACKPROP với payload đặc biệt 0xFFFC = Attention Mode */
  int err = bt_mesh_gradient_srv_backprop_send(&gradient_srv,
                                               (uint16_t)dest_addr,
                                               0xFFFC);

  if (err == 0) {
    shell_print(sh, "Lenh Attention da gui thanh cong!");
  } else if (err == -ENETUNREACH) {
    shell_error(sh, "Khong tim thay route den 0x%04x!", (uint16_t)dest_addr);
    shell_print(sh, "Dung 'mesh dest' de xem danh sach destination.");
  } else if (err == -EINVAL) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
    ...
    SHELL_CMD_ARG(fwd, NULL, "In Forwarding Table (danh sach neighbor)",
                  cmd_mesh_fwd, 1, 0),

    SHELL_CMD_ARG(rrt, NULL, "In Reverse Routing Table (bang dinh tuyen nguoc)",
                  cmd_mesh_rrt, 1, 0),

    SHELL_CMD_ARG(dest, NULL, "Liet ke tat ca destination co the gui BACKPROP",
                  cmd_mesh_dest, 1, 0),
    ...
    SHELL_CMD_ARG(attention, NULL,
                  "Bat Attention Mode: mesh attention <dest_addr>\n"
                  "  Vi du: mesh attention 0x0003",
                  cmd_mesh_attention, 2, 0),
